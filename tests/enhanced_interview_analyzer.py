...
import torch
import pickle  # Add pickle for model serialization
...
class AdvancedInterviewAnalyzer:
    def __init__(self):
        ...
        os.makedirs(self.facial_model_dir, exist_ok=True)
        os.makedirs(self.audio_model_dir, exist_ok=True)
        os.makedirs(self.nlp_model_dir, exist_ok=True)
        
        # Model file paths
        self.nlp_model_path = os.path.join(self.nlp_model_dir, "sentiment_model.pkl")
        ...
        # Load NLP model (first try to load from saved file, then from pre-trained model)
        self.load_nlp_model()
    ...
    def load_nlp_model(self):
        """Load NLP model from saved file or download pre-trained model"""
        self.nlp_model = None
        self.using_pipeline = False
        
        # First try to load from pickle file
        if os.path.exists(self.nlp_model_path):
            try:
                print(f"Loading sentiment analysis model from {self.nlp_model_path}...")
                with open(self.nlp_model_path, 'rb') as f:
                    self.nlp_model = pickle.load(f)
                self.using_pipeline = True
                print("Sentiment analysis model loaded successfully from saved file")
                return
            except Exception as e:
                print(f"Error loading model from file: {e}")
                # If loading fails, delete the corrupted file
                try:
                    os.remove(self.nlp_model_path)
                    print(f"Removed corrupted model file: {self.nlp_model_path}")
                except:
                    pass
        
        # If no saved model, download pre-trained model
        try:
    ...
    def save_nlp_model(self):
        """Save the NLP model to a pickle file"""
        if self.nlp_model is not None:
            try:
                print(f"Saving sentiment analysis model to {self.nlp_model_path}...")
                with open(self.nlp_model_path, 'wb') as f:
    ...
    def setup_ui(self):
        ...
        self.save_model_button = ttk.Button(button_frame, text="Save Model", style="TButton", command=self.save_nlp_model)
        self.save_model_button.pack(side=tk.LEFT, padx=5, pady=5, ipadx=10, ipady=5)
